...
class PurchaseSingleAllDetails {
  final int id;
  final String? userId;
  ...
  factory PurchaseSingleAllDetails.fromJson(Map<String, dynamic> json) {
    final data = json['data'] ?? json;

    return PurchaseSingleAllDetails(
      id: data['id'] ?? 0,
      ...
      items: (data['items'] as List?)?.map((item) => PurchaseItem.fromJson(item)).toList() ?? [],
      payments: (data['payments'] as List?)?.map((payment) => PurchasePayment.fromJson(payment)).toList() ?? [],
    );
  }
