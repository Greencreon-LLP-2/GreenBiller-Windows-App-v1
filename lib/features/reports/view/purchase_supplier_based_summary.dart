...
class PurchaseSupplierBasedSummary extends GetView<PurchaseSupplierSummaryController> {
  const PurchaseSupplierBasedSummary({super.key});
      ...
      child: Obx(() => DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              items: [
                ...
                ...controller.storeMap.entries.map((entry) {
              ...
              onChanged: (value) {
                ...
                controller.selectedStoreId.value =
                    value != null ? controller.storeMap[value] : null;
                controller.selectedSupplier.value = null;
                controller.selectedSupplierId.value = null;
                print('Store selected: $value, ID: ${controller.selectedStoreId.value}');
              },
            ),
          )),
    );
    ...
    return Container(
      ...
      child: Obx(() => DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              isExpanded: true,
              icon: controller.isLoadingSuppliers.value
                  ? const SizedBox(
                      width: 20,
                      height: 20,
              ...
              items: [
                ...
                ...controller.supplierMap.entries.map((entry) => DropdownMenuItem<String>(
                      value: entry.key,
                      child: Text(
                        entry.key,
                        style: const TextStyle(
              ...
              onChanged: (value) {
                ...
                controller.selectedSupplierId.value =
                    value != null ? controller.supplierMap[value] : null;
                print(
                    'Supplier selected: $value, ID: ${controller.selectedSupplierId.value}');
              },
            ),
          )),
    );
  ...
  Widget _buildTableRow(SignlePurchaseSummaryBySupplierModel data) {
    return MouseRegion(
                                ...
                                children: [
                                  ElevatedButton.icon(
                                    onPressed: () async {
                                      try {
                                        final file =
                                            await controller.generateSupplierSummaryPDF(
                                          data: data,
                                  ...
                                  ElevatedButton.icon(
                                    onPressed: () async {
                                      try {
                                        final file =
                                            await controller.generateSupplierSummaryCSV(
                                          data: data,
                                          startDate: controller.startDate.value!,
                                          endDate: controller.endDate.value!,
                                        );
                                        await OpenFile.open(file.path);
                                        print('CSV generated and opened: ${file.path}');
                                      } catch (e) {
                                        Get.snackbar(
                                          'Error',
