...
import 'package:green_biller/features/reports/sales_report/controllers/sales_summary_controller.dart';
...
class SalesSummaryPage extends HookConsumerWidget {
  ...
  Future<SalesSummaryModel> _generateReport(
    ...
    try {
      final dateFormat = DateFormat('yyyy-MM-dd'); // Server-friendly format
      final formattedStartDate = dateFormat.format(startDate);
      final formattedEndDate = dateFormat.format(endDate);

      final response = await SalesSummaryController().getSalesSummaryController(
        accessToken,
        formattedStartDate,
        formattedEndDate,
        null,
  ...
  Future<File> generateSalesSummaryPDF({
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Table(
            ...
            columnWidths: {
              0: const pw.FlexColumnWidth(1.2), // Sales Date
              1: const pw.FlexColumnWidth(1), // Sales ID
              2: const pw.FlexColumnWidth(2), // Store Name
              3: const pw.FlexColumnWidth(2), // Customer Name
    ...
    final file = File(
        "${output.path}/sales_summary_${DateTime.now().millisecondsSinceEpoch}.pdf");
    await file.writeAsBytes(await pdf.save());
    return file;
  }

  Future<File> generateSalesSummaryCSV({
    required List<Datum> data,
    required DateTime startDate,
    required DateTime endDate,
  }) async {
    ...
    final output = await getApplicationDocumentsDirectory();
    final file = File(
        "${output.path}/sales_summary_${DateTime.now().millisecondsSinceEpoch}.csv");
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final reportFuture = useState<Future<SalesSummaryModel>?>(null);
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: FutureBuilder<SalesSummaryModel>(
